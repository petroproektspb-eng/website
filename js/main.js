...
(() => {
    ...
    const heatDot = document.querySelector('.sigil-dot');
    ...
    function updateHeatSignature() {
        ...
        const hue = 27 + travel * (204 - 27);

        document.documentElement.style.setProperty('--heat-hue', hue.toFixed(2));

        const rail = document.querySelector('.scroll-sigil');
        if (!rail) return;

        const railHeight = rail.offsetHeight - 20;
        const dotPosition = 10 + travel * (railHeight - 22);
        heatDot.style.top = `${dotPosition}px`;
    }

    updateHeatSignature();
    window.addEventListener('scroll', updateHeatSignature, { passive: true });
    window.addEventListener('resize', updateHeatSignature);
